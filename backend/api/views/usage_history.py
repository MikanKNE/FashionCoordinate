...
def usage_list_create(request):
    ...
    # -------------------------------------------
        ...
        try:
            ...
            # ---- item_id の所有チェック ----
            owned_items = supabase.table("items")\
                .select("item_id")\
                .eq("user_id", user_id)\
                .execute()
            ...
            inserted = supabase.table("usage_history").insert(insert_rows).execute()
...
def usage_detail(request, history_id):
    ...
    try:
        ...
        existing = supabase.table("usage_history")\
            .select("*, items!inner(*)")\
        ...
        if not existing.data:
            return Response({"status": "error", "message": "Usage not found or no permission"}, status=404)
        ...
        # ------------------ PUT ------------------
            ...
            updated = supabase.table("usage_history")\
                .update(data)\
                .eq("history_id", history_id)\
                .execute()
